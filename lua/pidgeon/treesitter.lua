...
function M.getNearest(bufnr)
  ...
  vim.notify('no evaluable expression', vim.log.levels.WARN)
  return nil
end

function M.highlight(node)
  local name = 'pidgeon_highlight'
  ...
  vim.highlight.range(
    ...
    ns,
    'Visual',
    { node.range.startRow, node.range.startCol },
    { node.range.endRow, node.range.endCol },
    {}
  )

  -- Return a function which may be used to clear the highlight
  return function()
